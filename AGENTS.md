...
25. **Use Docker-in-Docker for security isolation** - Never mount the host Docker socket (`-v /var/run/docker.sock:/var/run/docker.sock`) into containers, even with rootless Docker. Mounting the host socket allows container escape via `docker run -v /:/host ubuntu cat /host/etc/passwd`. Instead, always use Docker-in-Docker (DinD) with `--privileged` and `DEVBOX_DOCKER_IN_DOCKER=true`. The inner daemon can only access the container's filesystem, not the host. This provides complete isolation while still allowing Docker usage inside containers.
