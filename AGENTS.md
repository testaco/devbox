...
34. **Store container configuration in labels (read-only after creation)** - Use Docker container labels (`devbox.egress`, `devbox.mode`, etc.) to persist configuration set at creation time. Labels are queryable via `docker inspect` and enable features like `devbox list` showing egress profiles. **Important**: Docker does NOT support adding/modifying labels on running containers - `docker container update --label-add` is not a valid command. Labels must be set at container creation time. For runtime-modifiable configuration, use file-based storage instead (e.g., `~/.devbox/egress-rules/<container>/` for custom network rules).
...
39. **Use static IP assignment for DNS proxy restarts** - Docker's `--dns` flag is set at container creation time and cannot be changed on running containers. To allow dynamic DNS proxy updates (e.g., `devbox network reset`), use the `--ip` flag when starting the DNS proxy on a user-defined network. This assigns a specific IP address to the container. When restarting the DNS proxy with new configuration, specify the same IP (`--ip <old-ip>`) so the main container's DNS resolution continues to work without restart. The pattern: (1) Get current DNS proxy IP before removal, (2) Remove old DNS proxy, (3) Start new DNS proxy with `--ip <same-ip>`. Verify IP preservation in integration tests.
40. **File-based storage for runtime-modifiable container configuration** - When container configuration needs to be modified after creation (like custom egress rules added via `devbox network allow`), use file-based storage at `~/.devbox/<feature>/<container>/` rather than attempting to modify container labels. Structure files simply (one value per line) for easy reading/writing in bash. Provide cleanup functions to remove files when containers are deleted. Re-apply configuration from files during `cmd_start()` to handle stop/start cycles. This pattern works around Docker's limitation that labels are immutable after container creation.
41. **GitHub Pages deployment for Vite projects** - Deploy Vite + React apps to GitHub Pages using artifact-based deployment workflow. Key configuration: (1) Set `base` in vite.config.ts to support both local dev (`/`) and production (`/reponame/`) via environment variable `VITE_BASE_PATH`. (2) Use separate deployment workflow with `actions/configure-pages`, `actions/upload-pages-artifact`, and `actions/deploy-pages`. (3) Add CI job for build verification (TypeScript type check, Vite build, output verification). (4) Use path filters to only deploy when `web/` changes. Requires enabling GitHub Pages in repo settings with "GitHub Actions" as source.
